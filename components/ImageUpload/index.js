...
class ImageUpload extends React.Component {
    ...
    render() {
        const { key, label, required } = this.props.field;
        // Sử dụng destructuring để lấy ra helperText và bỏ qua các prop khác
        const { helperText, ...otherProps } = this.state;
        return (
          <>
            <div
              className={
            ...
            >
              <div
                className={'img ' + (this.props.preview[this.props.field.key] ? '' : 'show')}
                // Không truyền helperText cho phần tử div
                {...otherProps}
              >
                <Image
                  src={
              ...
              <input
                ...
                type='file'
                onChange={this.handleUpload}
                // Không truyền helperText cho phần tử input
                {...otherProps}
              />
              <p className='MuiFormHelperText-root'>
                {this.props._findKey >= 0 ? _errArr[this.props._findKey].msg : ''}
